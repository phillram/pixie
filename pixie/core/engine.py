...
class Engine:
    ...
    def _poll_until(self, check: Callable[[], Any], timeout: float, what: str) -> Any:
        ...
        while True:
            ...
            if now - announced >= IDLE_NOTICE_SECONDS:
                announced = now
                forever = " (waiting as long as it takes)" if timeout <= 0 else ""
                self.log(f"    still waiting for {what}, {now - started:.0f}s so far"
                         f"{forever}", "muted")
    ...
    def _do_wait_for_color(self, step: dict[str, Any]) -> str:
        ...
        found = self._poll_until(
            lambda: screen.color_present(
                x, y, target,
                float(step.get("tolerance", 30)),
                int(step.get("radius", 3)),
                step.get("mode", "any"),
            ) or None,
            float(step.get("timeout", 30.0)),
            f"RGB{target} at {x}, {y}",
        )
        if found is None:
            self.log(f"    color RGB{target} not seen at {x}, {y} "
                     f"(saw RGB{screen.pixel_color(x, y)})", "warn")
            return "timeout"
        self.last_match = (x, y)
    ...
    def _look_for_color(self, step: dict[str, Any],
        ...
        return screen.find_color(
            region,
            tuple(step["color"]),
            float(step.get("tolerance", 50)),
            int(step.get("min_pixels", 40)),
            match=step.get("match", "rgb"),
            min_saturation=int(step.get("min_saturation", 90)),
            min_brightness=int(step.get("min_brightness", 70)),
        )

    @staticmethod
    def _color_description(step: dict[str, Any]) -> str:
        target = tuple(step["color"])
        how = "hue of " if step.get("match", "rgb") == "hue" else ""
    ...
    def _do_wait_for_color_in_area(self, step: dict[str, Any]) -> str:
        ...
        target = tuple(step["color"])
        min_pixels = int(step.get("min_pixels", 40))
        if hit is None:
    ...
    def _do_wait_for_image(self, step: dict[str, Any]) -> str:
        match = self._find(step, float(step.get("timeout", 30.0)))
        if match is None:
            self.last_match = None
            self.log(f"    {Path(step['image']).name} did not appear", "warn")
            return "timeout"
    ...
    def _do_click_image(self, step: dict[str, Any]) -> str:
        match = self._find(step, float(step.get("timeout", 10.0)))
        if match is None:
            self.last_match = None
            self.log(f"    {Path(step['image']).name} did not appear", "warn")
            return "timeout"
    ...
    def _do_click_image_if_present(self, step: dict[str, Any]) -> str:
        match = self._find(step, float(step.get("timeout", 3.0)))
        if match is None:
            self.last_match = None
    ...
    def _do_press_key(self, step: dict[str, Any]) -> str:
        ...
        presses = int(step.get("presses", 1) or 1)
        suffix = "  [dry run]" if self.dry_run else ""
    ...
    def run(self, max_cycles: int | None = None) -> None:
        ...
        self.log(f"Running '{self.sequence.name}'. "
                 f"Press {self.sequence.settings.abort_key} to stop.")
